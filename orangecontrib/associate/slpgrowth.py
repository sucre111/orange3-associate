...
def _fp_tree(db, min_support, max_support):
    """
    ...
    # todo
    least_items = {item for item, support in item_support.items()
                   if support >= min_support and support < max_support}
    ...
    db = ((count, sorted(frequent_items.intersection(transaction),
                         key=sort_index))
          for count, transaction in db if least_items.intersection(transaction))

    # db = ((count, transaction) for count, transaction in db if not least_items.intersection(transaction))
...
def _freq_patterns_single(P, alpha, min_support, max_support):
    """ Yield subsets of P as (frequent itemset, support) """
    for itemset in _powerset(P):
        if itemset[-1][1] > max_support:
            continue
        yield alpha.union(i[0] for i in itemset), itemset[-1][1]


def _freq_patterns_multi(Q, alpha, min_support, max_support):
    ...
    for item, nodes in reversed(Q.node_links):
        ...
        yield beta, support
        tree, got_itemsets = _fp_tree(_prefix_paths(Q, nodes), min_support, max_support)
        if got_itemsets:
            for itemset, support in got_itemsets:
                yield beta.union(itemset), support
        elif tree is not None:
            yield from _fp_growth(tree, beta, min_support, max_support)


def _fp_growth(tree, alpha, min_support, max_support):
    """ FP-growth ([1], § 3.3, Algorithm 2). """
    # Single prefix path optimization ([1] § 3.1)
    P, Q = _single_prefix_path(tree) if len(tree) == 1 else ([], tree)
    # Return P×Q
    yield from _freq_patterns_single(P, alpha, min_support, max_support)
    for itemsetQ, supportQ in _freq_patterns_multi(Q, alpha, min_support, max_support):
        yield itemsetQ, supportQ
        for itemsetP, supportP in _freq_patterns_single(P, alpha, min_support, max_support):
            yield itemsetQ | itemsetP, supportQ


def frequent_itemsets(X, min_support=.2, max_support=.2):
    """
...
than 4%:
    ...
    tree, itemsets = _fp_tree(db, min_support, max_support)
    if itemsets:
        yield from itemsets
    if tree:
        yield from _fp_growth(tree, frozenset(), min_support, max_support)
